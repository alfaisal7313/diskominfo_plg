...
class User extends CI_Controller{
	...
	function edit_user($d){
		...
		$users = $this->m_data->read_users($d);
		$data  = array('user' => $this->m_data->edit_data($where,'user')->result(),
			"title" => 'Form Edit User',
			'error' => '',
			'action' => site_url('user/create_action'),
			'dd_kategori' => $this->m_data->dd_akses_data(),
			'kategori_selected' => $this->input->post('id_kategori_user') ? $this->input->post('id_kategori_user') : $users->id_kategori_user
			// untuk edit ganti '' menjadi data dari database misalnya $users->id_kategori_user
		);
		$this->load->view('admin/header',$data);
	...
	function update_user(){
		...
		window.location.href = '" . base_url() . "admin/user/list_user';
		</script>";
	...
	function list_user()
	{
	 ...
	 function json() {
        header('Content-Type: application/admin/json');
